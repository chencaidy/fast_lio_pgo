...
esekf::esekf() {
  normvec = std::make_shared<PointCloud>(100000, 1);
  laserCloudOri = std::make_shared<PointCloud>(100000, 1);
  corr_normvect = std::make_shared<PointCloud>(100000, 1);
...
// 前向传播 公式(4-8)
void esekf::predict(double &dt, Eigen::Matrix<double, 12, 12> &Q, const input_ikfom &i_in) {
  Eigen::Matrix<double, 24, 1> f_ = get_f(x_, i_in);    // 公式(3)的f
  Eigen::Matrix<double, 24, 24> f_x_ = df_dx(x_, i_in); // 公式(7)的df/dx
  Eigen::Matrix<double, 24, 12> f_w_ = df_dw(x_);       // 公式(7)的df/dw
...
// ESKF
void esekf::update_iterated_dyn_share_modified(double R, PointCloud::Ptr &feats_down_body, KD_TREE<PointType> &ikdtree,
  ...
  for (int i = -1; i < maximum_iter; i++) // maximum_iter是卡尔曼滤波的最大迭代次数
  {
    ...
    Eigen::Matrix<double, 24, 1> dx_ =
        K * dyn_share.h + (KH - Eigen::Matrix<double, 24, 24>::Identity()) * dx_new; // 公式(18)
...
// 对应公式(2)的f
Eigen::Matrix<double, 24, 1> esekf::get_f(state_ikfom s, input_ikfom in) {
...
Eigen::Matrix<double, 24, 24> esekf::df_dx(state_ikfom s, input_ikfom in) {
...
Eigen::Matrix<double, 24, 12> esekf::df_dw(state_ikfom s) {
...
// 计算每个特征点的残差及H矩阵
void esekf::h_share_model(dyn_share_datastruct &ekfom_data, PointCloud::Ptr &feats_down_body,
...
#pragma omp parallel for num_threads(8)
  for (int i = 0; i < feats_down_size; i++) // 遍历所有的特征点
  {
    ...
    Eigen::Vector3d p_global(x_.rot * (x_.offset_R_L_I * p_body + x_.offset_T_L_I) + x_.pos);
    ...
    if (esti_plane(pabcd, points_near, 0.1f)) {
      float pd2 = pabcd(0) * point_world.x + pabcd(1) * point_world.y + pabcd(2) * point_world.z +
                  pabcd(3);                                // 当前点到平面的距离
      float s = 1 - 0.9 * fabs(pd2) / sqrt(p_body.norm()); // 如果残差大于经验阈值，则认为该点是有效点
  ...
  int effect_feat_num = 0; // 有效特征点的数量
  for (int i = 0; i < feats_down_size; i++) {
    if (point_selected_surf[i]) // 对于满足要求的点
    {
      laserCloudOri->points[effect_feat_num] = feats_down_body->points[i]; // 把这些点重新存到laserCloudOri中
      corr_normvect->points[effect_feat_num] = normvec->points[i];         // 存储这些点对应的法向量和到平面的距离
      effect_feat_num++;
    }
  }
  ...
  for (int i = 0; i < effect_feat_num; i++) {
    Eigen::Vector3d point_(laserCloudOri->points[i].x, laserCloudOri->points[i].y, laserCloudOri->points[i].z);
    Eigen::Matrix3d point_crossmat;
    point_crossmat << SKEW_SYM_MATRX(point_);
    Eigen::Vector3d point_I_ = x_.offset_R_L_I * point_ + x_.offset_T_L_I;
    Eigen::Matrix3d point_I_crossmat;
    ...
    // 计算雅可比矩阵H
    Eigen::Vector3d C(x_.rot.matrix().transpose() * norm_vec);
    Eigen::Vector3d A(point_I_crossmat * C);
    if (extrinsic_est) {
      Eigen::Vector3d B(point_crossmat * x_.offset_R_L_I.matrix().transpose() * C);
...
// 广义减法
vectorized_state esekf::boxminus(state_ikfom x1, state_ikfom x2) {
...
template <typename T>
bool esekf::esti_plane(Eigen::Matrix<T, 4, 1> &pca_result, const PointVector &point, const T &threshold) {
