...
ROS >= Humble (Recommend to use ROS-Humble). [ROS Installation](https://docs.ros.org/en/humble/Installation.html)
...
source install/local_setup.bash # use local_setup.zsh if use zsh
```
...
1. Please make sure the IMU and LiDAR are **Synchronized**, that's important.
2. The warning message "Failed to find match for field 'time'." means the timestamps of each LiDAR points are missed in the rosbag file. That is important for the forward propagation and backwark propagation.
3. We recommend to set the **extrinsic_est_en** to false if the extrinsic is give. As for the extrinsic initiallization, please refer to our recent work: [**Robust Real-time LiDAR-inertial Initialization**](https://github.com/hku-mars/LiDAR_IMU_Init).

### 3.1 Run use ros2 launch

With a undistorted PointXYZI format **sensor_msgs::msg::PointCloud2** and **sensor_msgs::msg::Imu** topics, then

```bash
cd ros2_ws
source install/local_setup.bash # use local_setup.zsh if use zsh
ros2 launch fast_lio mapping.launch.py config_file:=undis.yaml
```
...
1. LiDAR point cloud topic name: ``` lidar_topic ```
2. IMU topic name: ``` imu_topic ```
3. LiDAR frame ID: ``` lidar_frame_id ```
4. IMU frame ID: ``` imu_frame_id ```
